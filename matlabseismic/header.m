...
%
%   The trace complete trace header of trace k is loaded
%   in a structure called header. This fucntion is used 
%   by readsegy.m
%
%   header = header(FID,ns,k);
%
%   IN   FID: file identifier number (set by fopen)
%        ns: numer of samples per trace (can be obatained using
%            extract.m)
%        k: trace number you want to retrieve the header from 
%
%   Example: header = header(FID,1024,10) gets the complete header
%                     of the trace 10 in the file FID (already opened with 
%                     FID=fopen(...)). The trace has 1024 samples. 
%
%
