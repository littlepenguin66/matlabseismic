function seis_env_dB(Data, Data_g, t, tnum)
    ...
    % September 2011.
    % Modified by: little_penguin66 Date: 04/09/2025 diff:Fixed the usage of the legend function in versions after R2017a and optimized the layout.
    ...
    if nargin == 4
        ...
        grid
        legend('Before gain correction', 'After gain correction', 'Location', 'northeast')
    else
        trace1 = mean(Data');
        trace1 = trace1 / max(trace1);
        trace1_env_dB = 20 * log10(abs(hilbert(trace1 + eps))); %log of envelop
        ...
        trace1_env_dB_towp = 20 * log10(abs(hilbert(trace1_towp + eps))); %log of envelop %check envelop
        figure
        plot(t, trace1_env_dB, 'b', t, trace1_env_dB_towp, 'r-.', 'LineWidth', 1)
        xlabel('Time (s)', 'FontSize', 14)
