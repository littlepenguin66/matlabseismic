...
%%
%Frequency-wavenumber (f-kx) representation of the data
[Data_fk,f,kx]=fk(Dshot,dt,dx);
%Plotting the magnitude spectrum in dB of the seismic f-kx representation
figure,
...
ylabel('Frequency (Hz)','FontSize',14)
set(gca,'xaxislocation','top')
axis([min(kx),max(kx),0,max(f)])%Diplaying +ve frequencies
x=10;y=10;%starting screen position
w=300;%figure width
h=600;%figure hieght
set(gcf,'position',[x y w h]);